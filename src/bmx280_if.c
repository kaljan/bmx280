...
int bmx280_i2c_write_byte(struct i2c_client *client, uint8_t reg, uint8_t data) {
    int ret;
    uint8_t buf[2] = {reg, data};
    ret = i2c_master_send(client, buf, 2);
    if (ret < 0) {
        dev_err(&client->dev, "i2c write byte failed\n");
    }
    return ret;
}

int bmx280_i2c_read(struct i2c_client *client, uint8_t reg, uint8_t* data, uint8_t size) {
    int ret = 0;
    ...
    if (ret < 0) {
        dev_err(&client->dev, "i2c read byte failed\n");
        return ret;
    }
    return ret;
}
...
int bmx280_modify_reg(struct i2c_client *client,
    ...
    if (bmx280_i2c_read(client, reg, &data, 1) <= 0) {
        dev_err(&client->dev, "read register 0x%02X failed\n", reg);
        return -1;
    }
    ...
    if (bmx280_i2c_write_byte(client, reg, data) <= 0) {
        dev_err(&client->dev, "write register 0x%02X failed\n", reg);
        return -1;
    }
    return 0;
}
...
int bmx280_modify_config_reg(struct i2c_client *client,
    ...
    if (bmx280_i2c_read(client, BMX280_CMR, &data, 1) <= 0) {
        dev_err(&client->dev, "read ctrl_meas(0xF4) register failed\n");
        return -1;
    } else if (data & BMX280_CMR) {
        dev_err(&client->dev, "write to config register only in sleep mode\n");
        return -1;
    }
